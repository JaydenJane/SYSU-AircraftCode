...
void FTC_IMU::DCM_CF(Vector3f gyro,Vector3f acc, float deltaT)
{
	...
	static Vector3f pregyro,curgyro;
	curgyro = (gyro + pregyro) * 0.5f * deltaT;
	pregyro = gyro;
	
	Matrix3f dcm;
	dcm.from_euler(curgyro);
	
	static Vector3f gra( 0 , 0 , ACC_1G ), y( 200 , 0 , 0 );
	gra = dcm * gra;
	y = dcm * y;
	
	if(50 < accRatio && accRatio < 150) gra = CF_1st( gra , acc , ftc.factor.gyro_cf );
	
	gra.get_rollpitch(angle);
	y.get_yaw(angle);
}
...
void FTC_IMU::Quaternion_CF(Vector3f gyro,Vector3f acc, float deltaT)
{
	//to do
	Vector3f V , error;
	static Vector3f error_Inte(0,0,0);
	acc.normalize();
	Q.vector_gravity(V);
	error = acc % V;
	error_Inte += error*Ki;
	gyro += error*Kp + error_Inte;
